...
# list comprehension verision to consider 
# def get_even_nums(nums):
...
Theis list represnts the ingredients used to make a sandwich, it's first and
...
def get_sandwich_filling(sandwich):
    ...
    for filling in sandwich:
        if filling != 'bread':
            sandwich_filling.append(filling)
    return sandwich_filling
            
    pass
#def get_sandwich_filling(sandwich):
    return [filling for filling in sandwich if filling != "bread"]
...
#def remove_item(items, n):
    new_list = []
    for i in range(len(items)):     # loop over indexes, not just values
        if i != n:                  # skip the element at position n
...
def is_item_omnipresent(lists, item):
    ...
    pass
# so we are looking in lists so for stuff in lists and that stuff has to has to be in each list and return boolean to show this 
...
def flatten_list_by_one(nested_lists):
    ...
    for sublist in nested_lists:
        flat_list.extend(sublist)  # add all elements from this sublist
    return flat_list
